...
def _normalizar_texto_memoria(texto: str) -> str:
    texto = texto.lower()
    texto = "".join(
...
class MemoriaSimple:
    ...
    def _contiene_dato_memorable(self, msgs: list[dict]) -> bool:
        texto = " ".join(str(m.get("text", "")) for m in msgs if m.get("role") == "user")
        if not texto:
            return False
        compacto = _normalizar_texto_memoria(texto)
        return any(keyword in compacto for keyword in MEMORIA_KEYWORDS_COMPACTAS)
    ...
    def agregar(self, role: str, text: str, detectar: bool = True) -> None:
        """
        ...
        if detectar and role == "model" and self._contiene_dato_memorable(self.memoria[-4:]):
            threading.Thread(
                target=self._detectar_datos_usuario,
                daemon=True
            ).start()

    def integrar_mensajes_guardados(self, mensajes: list[tuple[str, str]], detectar: bool = True) -> None:
        """
        Integra mensajes que ya fueron guardados en DB.
        La voz en vivo guarda cada transcripción al llegar; al cerrar solo se
        actualiza la ventana/resumen para no duplicar filas.
        """
        normalizados = [
            {"role": role, "text": (text or "").strip()}
            for role, text in mensajes
            if role in {"user", "model"} and (text or "").strip()
        ]
        if not normalizados:
            return

        self.memoria.extend(normalizados)
        while len(self.memoria) > self.max_mensajes:
            self._comprimir()

        if detectar and self._contiene_dato_memorable(normalizados):
            self._detectar_datos_usuario_con_msgs(normalizados[-6:])
    ...
    def detectar_desde_voz(self, transcripciones: list) -> None:
        ...
        if not self._contiene_dato_memorable(msgs):
            return
        threading.Thread(
